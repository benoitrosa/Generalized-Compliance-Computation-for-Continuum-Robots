...
% ======================================================================= %
    ...
    bool_back_st    = false ;                               % (boolean) Is the backbone is straight ? If not, it is circularly curved
    bool_tendon_st  = false ;                               % (boolean) Is the tendons are straight ? If not, they are helicoidally curved
    ...
    resolution              = 0.003  ;                      % [m] Discretization step away from discontinuity points (𝛥(𝑠) in the paper see Table 5)
    ...
    delta_f0            = [[-1 , 0.75 , 0.5];[ 0.5 , -1 , 0.25]] ;  % [N] Matrix of the forces applied on the CTCR (row i for vector force i)
    ...
    simulation_param            = SimulationParam(resolution , bool_display_iter , FunctionTolerance , StepTolerance , MaxIter , MaxFunctionEvaluations , bool_opt_lit , bool_SIC , nb_pt_dict , resol_pt_disct , pt_s0_LIT , bool_J , bool_Cs0 , bool_problem_opt , bool_disp_terminal , bool_back_st , bool_tendon_st) ;
