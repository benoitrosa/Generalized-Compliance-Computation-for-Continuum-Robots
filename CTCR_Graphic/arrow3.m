...
if ~(exist('fig') && exist('ax') && ~(isempty(fig)) && ~(isempty(ax)))
    fig=gcf; ax=gca;
end
...
if ~cone
  ...
  for i=1:n
    if lw(i)
      H1(i)=plot3(ax,[p1(i,1),p2(i,1)],[p1(i,2),p2(i,2)],...
...
G=surface(ax,x/2,y/2,z); dar=diag(ar);
...
if any(ip>0)
  ...
  G=surface(ax,x*ar(1)/2,y*ar(2)/2,z*ar(3)/2);
  set(G,'EdgeColor','none');
  X=get(G,'XData'); Y=get(G,'YData'); Z=get(G,'ZData');
  H3=zeros(n,1);
  for i=1:n                                        % translate
